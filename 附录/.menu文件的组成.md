...
menu 的数据段会根据 “**类别(category)**” 不同而写法不同，制作物品 MOD 时，最好参考同一类别已有的示例进行编写。
...
1. **字段之间用 Tab 键分隔**。  
2. **行首若是 `;(半角分号)`、`/(单斜杠x1)` 或 `//(单斜杠x2)`** 则该行为注释，不会被执行。  
3. **icons 字段若被删除**，则该物品在衣装列表中不可见（官方的“翻起裙子”menu 等就有使用这种方法）。  
...
- 一般来说会写本 `.menu` 文件所在的完整路径，但实际只要末尾文件名一致就能正常识别，比如 `xxx.menu.txt`。
...
folder_underhair	underhair	moza	seieki_hara	folder_skin	skin
