...
如果你不懂什么是骨骼、权重，请先学一下这个视频（虽然是某种搬运 AI 中文配音） [https://www.bilibili.com/video/BV1YCJwz8EwH/](https://www.bilibili.com/video/BV1YCJwz8EwH/)
...
提示 `ウェイトが割り当てられていない頂点が見つかりました、中止します` 说明你用了无效的骨骼，或者没有正确设置顶点组和权重
...
为了节省资源，游戏里内表面一般是不渲染的，看起来就直接是透明的，或者是黑的（取决于材质/着色器）。

所以我们需要把它改成外表面

但是呢，这是个一维平面，2 个面不能都是外表面，虽然 blender 里面可以选择双面渲染

但是我前面也说了，你可以忘掉学到的 blender 材质了，游戏内的渲染是由游戏决定的（取决于材质/着色器），你在 blender 调的渲染方式什么用都没有
