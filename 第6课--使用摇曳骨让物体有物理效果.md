![图片](https://github.com/user-attachments/assets/55904afe-50f3-404c-b311-7d309f4d821c)# 第 6 课，使用摇曳骨让物体有物理效果
...
不过由于 KISS 延续 10 年的旧代码过于神秘，我不敢说这个分析 100％ 正确。
...
进入游戏中，点击位置设定按钮，然后拖动，可以看到它有物理效果了！
...
我们首先需要一个 .col 和 .phy 文件，DynBoneEdit 才能识别

<br>

打开 COM3D2 MOD EDITOR V2，到 Phy 和 Col 选项中直接点击另存为，这样你就得到了一份空的 .phy 和 .col 文件。

如果你不需要碰撞箱，那么 .col 文件可以不需要。
...
游戏读取这些文件的方式详见 《游戏如何读取 .phy 和 .col》 章节
...
由于 DynBoneEdit 的 BUG 宇宙无敌多，时常会有无法导出的情况，而且编辑 .phy 时无法实时预览，只有编辑碰撞器时可以，因此仅建议使用 DynBoneEdit 编辑碰撞器，如果遇到任何导出不了，或者导出了但没有效果，请使用编辑器解决。
...
然后这里就是最难的地方拉，这个碰撞器真的很拉跨，要调处好的效果需要很久很久，而且 DynBoneEdit 的预览也不是很准，所以做好心理准备。
