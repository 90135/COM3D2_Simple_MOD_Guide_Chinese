...
- 最简单来说就是 物体名 = Blender 中的物体名称（黄色倒三角）
...
`_yure_hard_` 和 `_yure_soft_` 没有默认 .phy 文件，它们的计算方式似乎和其他骨骼不同。

在 COM3D2 MOD EDITOR V2 的 Phy 编辑中提到 
...
模式 2 的骨骼关键词 `_h_` 或 `_h50_` 的参数设置和 `_yure_hard_` 和 `_yure_soft_` 的在一个地方，因此推测这也是 Dynamic Bone 出现前的旧代码。
...
如果你不需要碰撞箱，那么 .col 文件可以不需要，如果你只准备了 .phy，你会发现在 DynBoneEdit 中添加碰撞器是没有效果的。
